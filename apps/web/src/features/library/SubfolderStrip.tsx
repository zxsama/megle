import { Folder, FolderOpen, PanelTopOpen, PanelTopClose } from "lucide-react";
...
export function SubfolderStrip({
  ...
  return (
    <section
      className={`subfolder-strip${open ? " is-open" : ""}`}
    ...
    >
      <header className="subfolder-strip-header">
        ...
        <LiquidGlassButton
          aria-label={open ? "Hide child folders" : "Show child folders"}
          aria-pressed={open}
          active={open}
          className="subfolder-strip-toggle"
          onClick={onToggleOpen}
          title={open ? "Hide child folders" : "Show child folders"}
          tone="control"
          type="button"
        >
          {open ? <PanelTopClose aria-hidden="true" size={15} /> : <PanelTopOpen aria-hidden="true" size={15} />}
        </LiquidGlassButton>
      </header>
      {open ? (
